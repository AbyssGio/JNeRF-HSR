...
class IntegratedPositionEncoder(nn.Module):
    ...
    def expected_sin(self, x, x_var):
        ...
        y_var = jt.maximum(
            jt.zeros_like(x),
            0.5 * (1 - jt.exp(-2 * x_var) * jt.cos(2 * x)) - y**2
        )
        return y, y_var

    def execute(self, x, x_cov, diag=True):
        ...
        return self.expected_sin(
            jt.concat([y, y + 0.5 * math.pi], -1),
            jt.concat([y_var, y_var], -1)
        )[0]
